/* numberAsWords.js as of 2017/02/16/23:01:00 */
...
namespace.replaceNumbersOnPage = {};
var replaceNumbersOnPage = namespace.replaceNumbersOnPage;

replaceNumbersOnPage.isElement = function(obj)
{
	try
	{
...
replaceNumbersOnPage.replaceNumbersInTextNode = function (textNode)
{
	var match;
	
	//match any number of commas, possibly followed by a character we wouldn't want to put a dash after, followed by any number of digits and/or commas followed by a digit, possibly followed by a character we would want to put a dash before
	var findNumberAndInterestingPrecedentRegex = /(,*)([^\d\s\.\:\$])?([\d,]*\d)+([^\d\s\.\:\$\,])?/;
	
	//stores the entire match, the preceeding commas, the interesting preceeding character, and the number in an array
	match = textNode.data.match(findNumberAndInterestingPrecedentRegex);
	
	//while there is still something being matched, 
	while (match !== null)
	{
		//replace the match with the captured comma (if it exists), preceeding character, a hyphen, and the number as a word
		//the parenthesis around the statement with the ternary operator are to prevent the following string from being considered the "false" condition
		textNode.data = textNode.data.replace(match[0], (match[1]?match[1]:"") + (match[2]?match[2] + "-":"") + numberAsWords.charactersAsWords(match[3]) + (match[4]?"-" + match[4]:""));
		
		//same regex match as above
		match = textNode.data.match(findNumberAndInterestingPrecedentRegex);
	}
...
replaceNumbersOnPage.alterAllTextNodesUnderNode = function (node, alteringFunction)
{
	//a TreeWalker that finds the next text node
	var walker = document.createTreeWalker(node, NodeFilter.SHOW_TEXT, null, false);
	//the current node found by the TreeWalker
	var currentTextNode;
	
	while(currentTextNode = walker.nextNode())
	{
		alteringFunction(currentTextNode);
	}
...
replaceNumbersOnPage.replaceNumbersOnPage = function ()
{
	var thisDocumentBody = document.body;
	
	if (replaceNumbersOnPage.isElement(thisDocumentBody))
	{
		//replace numerals with words
		return replaceNumbersOnPage.alterAllTextNodesUnderNode(document.body, replaceNumbersOnPage.replaceNumbersInTextNode);
	}
	else
	{
		return false;
...
//"main" function
//just calls replaceNumbersOnPage()
replaceNumbersOnPage.main = function ()
{
	return replaceNumbersOnPage.replaceNumbersOnPage();
}

//run the main function
replaceNumbersOnPage.main();
